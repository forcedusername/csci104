...
template <class T>
class Stack : public DoublingArrayList<T> /* may add inheritance declarations here 
            - decide if inheritance is private or public */
{
  ...
  // add appropriate constructors and destructors
		Stack(){};
		~Stack(){};
		using DoublingArrayList<T>::isEmpty;
		using DoublingArrayList<T>::push_back;
		using DoublingArrayList<T>::remove;
		using DoublingArrayList<T>::current_size;
		using DoublingArrayList<T>::capacity;
		//using PlusTenArrayList<T>::array_locate;
		using DoublingArrayList<T>::get;
