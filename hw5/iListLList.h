...
class List : public IList <T> {
  public:
    ...
    List (const List<T> &other) {
      // copy from other list to this list; 
      //  you st
      int size= other.size ( );
      int iterator=0;
      
      while (iterator<size){
        T taker= other.get (iterator);
        insert (0, taker);
        iterator++;
      }
    ...
    //head and tail new need to be created and you have a new list item equal to yours
	//loop through other get all the elements and then assign each element to 
	//the list via insert function
...
template <class T>
T const & List<T>:: get (int position) const{
	Item<T>* head_copy =head;
	if (position <0 || position >= size())
		throw position;
	for (int i=0; i<position;i++){
